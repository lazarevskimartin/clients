...
const STREET_COLORS: Record<string, string> = {
  'Венјамин Манџуковски': '#ffe0b2', // light orange-peach
...
const ClientCard: React.FC<ClientCardProps> = ({ client, onDelete }) => {
    return (
        <Card sx={{ mb: 1, background: STREET_COLORS[(client.address || '').split(' ').slice(0, -1).join(' ')] || '#f1f3f4', borderRadius: 2 }}>
            <CardContent>
                ...
                <Box sx={{ display: 'flex', gap: 1, mt: 1 }}>
                    ...
                    {onDelete && (
                        <IconButton color="error" onClick={() => onDelete(client._id)}>
                            <DeleteIcon />
                        </IconButton>
                    )}
                </Box>
